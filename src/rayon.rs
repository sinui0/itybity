...
use crate::{BitIter, BitLength, BitOrder, GetBit, Lsb0, Msb0};
...
where
    T: GetBit<Lsb0> + GetBit<Msb0> + BitLength + Clone + Send,
{
...
mod tests {
    ...
    where
        T: Fixtures<T> + IntoParallelBits + Copy + std::fmt::Binary,
    {
        ...
        T: Fixtures<T> + IntoParallelBits + Copy + std::fmt::Binary,
    {
        ...
        assert_eq!(
            slice.into_par_iter_msb0().collect::<Vec<bool>>(),
            expected_msb0_bits
        );
        assert_eq!(
            slice.into_par_iter_lsb0().collect::<Vec<bool>>(),
            expected_lsb0_bits
        );
    }
}
